<div class="mx-4">
        ...
        <div class="flex flex-col justify-between mb-4 bg-simipa-2 p-4 min-h-[20vh] rounded-md sm:min-h-[30vh] sm:p-8">
            ...
            <div class="flex gap-2 items-center justify-between">
                <p class="text-simipa-5 sm:font-medium sm:text-xl">
                    Kode Penilaian: </p>
                <p class="text-simipa-5 sm:font-medium sm:text-xl">{{ $assessmentType->code }}
                </p>
        ...
        <div class="mb-[15vh] ">
            @foreach ($studentScores as $s)
                <div class="flex justify-between items-center mb-3 border-2 border-slate-400 p-2 rounded-md">
                    <div>
                        <p class="font-medium text-sm text-simipa-2 sm:text-md">{{ $s['student_name'] }}</p>
                        <p class="text-xs font-light">NISN: {{ $s['student_nisn'] }}</p>
                    </div>

                    <div>
                        <button
                            class="px-3 py-1 rounded-lg bg-simipa-1 text-simipa-5 min-w-[20vw] sm:min-w-[10vw] hover:bg-simipa-2 sm:px-6 sm:py-2 hover:cursor-pointer">{{ $s['score'] }}
                        </button>
                    </div>
                </div>
            @endforeach
    ...
    <livewire:teacher.assessment.create.score-form />
